<template>
  <div id="eventSettings-form">
    <b-list-group>
      <b-list-group-item style="padding: 0rem 0.2rem;" v-for="item in eventsParams" v-bind:key="item.id">
...
export default {
  ...
  methods: {
    ...
    onEventParamsChange() {
      ...
      eventActions.setValuesFromParams(this.eventsParams);
    },
    onResetEventParams() {
      console.log("onResetEventParams");
      for (let i = 0; i < this.eventsParams.length; i++) {
        this.eventsParams[i].value = this.eventsParams[i].defaultValue;
      }
